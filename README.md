...
GPU, CoreText for glyph outlines, and AudioToolbox for the speaker.

Around 65,000 lines, 69 test targets, everything built with Bazel.
...
takes 6000 objects from 3428 draw calls at 3.38 ms to 3 draws at 0.98 ms *in a
scene of 6000 copies of one mesh* (`tests/manyobjects`). On the demo valley,
whose 196 batches are mostly single-instance, it wins by 8% at `--crowd 600` and
loses by 13% at `--crowd 6000`: the per-batch cull dispatches cost more than the
draws they save once the batches stop being large. `--indirect` is a flag, not
the default, and the honest numbers are in the commit that says so.
...
accessors; and WAV. There is no runtime content pipeline yet: every mesh,
texture and sound in `apps/world` is generated by C++ at startup, and the
disk-reading half of the importer stack has no caller outside its own tests.
...
one event rather than five and never a half-written file. Both are tested and
neither has a user: nothing writes a package and nothing loads one, so this is
a pipeline with no ends rather than a pipeline.
...
queue, a job system that gets 10.9x on sixteen threads, and gamepad support with a
radial deadzone. Per-pass GPU timings, as a timeline rather than a column of
durations — a tile-based GPU overlaps passes, so eleven of them sum to 16.7 ms
of a 4 ms frame, and only begin-and-end shows which are actually on the critical
path.
